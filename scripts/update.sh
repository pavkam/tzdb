...
IANAV=$(cat ./iana_temp/version) || error "Failed to extract the IANA version."
info "Current TZDB database version is v$IANAV."
...
./bin/TZCompile ./tz_database_latest $TZDB_INC.temp $IANAV > $LOG_FILE 2>&1 || error "Failed to process latest TZDB data."
...
# Update the README
info "Updating README with the new version '$IANAV'..."
README=./README.md
cat $README | sed "s/\(.*\*\*\)[0-9]*[a-z]*\(\*\*.*\)/\1$IANAV\2/g" > $README.tmp 2> $LOG_FILE || error "Failed to update README.md file with the IANA DB version."
...
if [ $CI -eq 1 ]; then
    ...
    git commit -m "chore: bump version" > $LOG_FILE 2>&1 || error "Failed to commit the changes to the remote repository."
